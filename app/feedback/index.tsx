...
export default function TelaFeedback() {
  ...
  useEffect(() => {
    const carregarAtividades = async () => {
      try {
        const dados = await apiProgramacao.buscarAtividades();
        setAtividades(dados);
  ...
  const filtrarAtividades = async (tipo: string) => {
    ...
    try {
      if (tipo === 'Todos') {
        ...
        const dados = await apiProgramacao.buscarAtividades(tipo);
        setAtividadesFiltradas(dados);
      }
    } catch (erro) {
  ...
  const manipularPressionarAtividade = (atividade: Atividade) => {
    router.push(`/feedback/${atividade.id}`);
  };

  const irParaMinhasAvaliacoes = () => {
    router.push('/feedback/minhas-avaliacoes');
  };
    ...
    return (
      ...
      >
        <View style={styles.infoAtividade}>
          ...
          <View style={styles.containerBotaoAvaliacoes}>
            <Text style={styles.textoBotaoAvaliacoes}>
              Ver avaliações do evento →
            </Text>
          </View>
        </View>
      </TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity 
          style={styles.botaoMinhasAvaliacoes}
          onPress={irParaMinhasAvaliacoes}
        >
          <Text style={styles.textoMinhasAvaliacoes}>Minhas Avaliações</Text>
        </TouchableOpacity>
      </View>
      ...
      {carregando ? (
        <View style={styles.containerCarregando}>
          ...
          <Text style={styles.textoCarregando}>Carregando palestras...</Text>
        </View>
      ) : (
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.containerVazio}>
              <Text style={styles.textoVazio}>
                Nenhuma palestra encontrada para o filtro selecionado.
              </Text>
            </View>
          }
        />
...
const styles = StyleSheet.create({
  ...
  botaoMinhasAvaliacoes: {
    backgroundColor: '#10B981',
    paddingHorizontal: 16,
    paddingVertical: 8,
    borderRadius: 20,
  },
  textoMinhasAvaliacoes: {
    color: '#FFFFFF',
    fontSize: 14,
    fontWeight: '600',
  },
